...
void GLCM::CalcuTextureEValue(Mat src, TextureEValues& EValue, int size, GrayLevel level)
{
    ...
    EValue.contrast =EValue.contrast/(imgGray.rows*imgGray.cols);
    EValue.energy =EValue.energy/(imgGray.rows*imgGray.cols);
    EValue.entropy =EValue.entropy/(imgGray.rows*imgGray.cols);
    EValue.homogenity =EValue.homogenity/(imgGray.rows*imgGray.cols);
}
